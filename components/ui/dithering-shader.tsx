...
void main() {
  ...
  if (u_shape < 1.5) {
    ...
    shape_uv *= 4.;
    float wave  =       snoise(vec2(shape_uv.x * .35,        t * .45));
    wave       += .5  * snoise(vec2(shape_uv.x * .85 + 1.7,  t * .35));
    wave       += .25 * snoise(vec2(shape_uv.x * 1.7 - 0.9,  t * .55));
    shape = 1. - smoothstep(-1., 1., shape_uv.y + wave * .5);
  } else if (u_shape < 5.5) {
...
export function DitheringShader({
  ...
  // Resize canvas to match container so dither pixels are always square
  useEffect(() => {
